...
import Parernt from "@/app/models/Parent";
...
export const PATCH = async (request, {params}) => {
  try {
    ...
    const parent = await Parernt.findByIdAndUpdate(params._id, image, {
      new: true,
      runValidators: true,
    });
    ...
    return new Response(JSON.stringify({ status: "success", data: parent.profile_image }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (error) {
