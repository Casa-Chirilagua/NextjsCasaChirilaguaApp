...
/**
 * Search families by search query
 */
export const searchFamilies = createAsyncThunk('families/search', async (query, thunkAPI) => {
  try {
    return await familiesService.searchFamilies(query);
  } catch (error) {
    const message = HandleMessage(error);
    return thunkAPI.rejectWithValue(message);
  }
...
export const familiesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      builder
      .addCase(searchFamilies.pending, (state) => {
        state.isLoading = true;
        state.isError = null;
      })
      .addCase(searchFamilies.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        state.families = action.payload;
      })
      .addCase(searchFamilies.rejected, (state, action) => {
        state.isLoading = false;
        state.isError = true;
        state.message = action.error.message;
      });
